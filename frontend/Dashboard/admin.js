document.addEventListener("DOMContentLoaded", () => {
    ...
    if (typeof renderMenu === 'function') renderMenu("admin");
    ...
    (async () => {
        try {
            ...
            if (emptyList.length === 0) {
                phongTrongTable.innerHTML = '<tr><td colspan="3">✅ Không có phòng trống</td></tr>';
            } else {
                phongTrongTable.innerHTML = emptyList.map(r => `
                    <tr><td><b>${r.name}</b></td><td>${r.building}</td><td>${r.price.toLocaleString()} đ</td></tr>
                `).join("");
            }
            ...
            document.getElementById("todoList").innerHTML = `
                ...
                <li>📋 Kiểm tra hợp đồng sắp hết hạn</li>
            `;
